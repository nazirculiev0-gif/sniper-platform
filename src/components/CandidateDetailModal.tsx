  ...
  const saveProfile = async () => {
    ...
    const res = await fetch(`/api/candidates/${candidate.id}`, {
      ...
      body: JSON.stringify({
        ...
        desiredPositions: pDesiredPositions.split(",").map((s: string) => s.trim()).filter(Boolean),
        ...
        skills: pSkills.split(",").map((s: string) => s.trim()).filter(Boolean),
        languages: pLanguages.split(",").map((s: string) => s.trim()).filter(Boolean),
        tags: pTags.split(",").map((s: string) => s.trim()).filter(Boolean),
      ...
      <div className="card" style={{ width: "min(640px, 100%)", maxHeight: "85vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: 18 }}>
          ...
          {tab === "chat" && (
            <div className="mini muted">
              {candidate.requestId
                ? "Переписка ведётся в общем чате заявки — откройте вкладку «Чат» на странице заявки."
                : "Кандидат пока не привязан ни к одной заявке — переписка появится, когда вы добавите его в конкретную вакансию."}
            </div>
          )}
